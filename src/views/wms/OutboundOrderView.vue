  ...
  <div class="wms-page">
    <header class="page-header">
      <div class="page-header__main">
        <p class="page-header__eyebrow">Outbound Workspace</p>
        <h1>出库管理</h1>
        <p>围绕“草稿创建 → 提交 → 拣货发货 → 完成出库”组织单据、状态和库存扣减动作。</p>
      </div>
      <div class="actions-row page-header__actions">
        <RouterLink to="/dashboard"><el-button plain>返回首页</el-button></RouterLink>
        <el-button type="primary" @click="openCreateDialog">新建出库单</el-button>
      </div>
    </header>
    ...
    <section class="panel page-flow">
      <div class="panel-header">
        <div>
          <h3>出库流程</h3>
          <p class="panel-subtext">单据驱动库存减少，完成发货后同步生成库存流水。</p>
        </div>
      </div>
    ...
    <section class="panel">
      <div class="toolbar-row">
        <div class="toolbar-row__left">
          <div>
            <h3 class="panel-title-inline">出库单列表</h3>
            <p class="toolbar-row__meta">列表页承担查询、查看、编辑草稿和触发发货动作。</p>
          </div>
        </div>
      ...
      <el-table :data="rows" v-loading="loading" empty-text="暂无数据">
        ...
        <el-table-column label="出库时间" min-width="170">
          <template #default="{ row }">{{ row.outboundTime || '-' }}</template>
        </el-table-column>
    ...
    <el-dialog
      ...
      class="wms-dialog"
      :title="dialogMode === 'create' ? '新建出库单草稿' : '编辑出库单表头'"
      width="980px"
...
import { Box, Document, Van, WarningFilled } from '@element-plus/icons-vue'
...
const flowSteps = [
  { key: 'draft', step: 'STEP 01', title: '创建草稿', meta: '填写出库单头与业务说明', tone: 'flow-node--primary' },
  { key: 'items', step: 'STEP 02', title: '补充明细', meta: '选择物料、库位和计划数量', tone: '' },
  { key: 'submit', step: 'STEP 03', title: '提交校验', meta: '校验库存与发货条件', tone: 'flow-node--warning' },
  { key: 'ship', step: 'STEP 04', title: '拣货发货', meta: '执行扣减并登记结果', tone: 'flow-node--success' },
  { key: 'finish', step: 'STEP 05', title: '完成出库', meta: '库存减少并生成流水', tone: 'flow-node--success' }
] as const
...
const metrics = computed(() => {
  ...
  return [
    ...
    { key: 'draft', label: '草稿单据', value: String(draft), meta: '待提交或待发货', color: '#ff9a1f', icon: WarningFilled },
    { key: 'confirmed', label: '已确认单据', value: String(confirmed), meta: '已执行出库动作', color: '#34b36b', icon: Van },
...
async function confirmRow(row: OutboundOrder): Promise<void> {
  clearMessages()
  try {
    await ElMessageBox.confirm(`确认将出库单 ${row.orderNo} 执行“确认”吗？`, '确认出库单', {
      type: 'warning',
...
async function removeRow(row: OutboundOrder): Promise<void> {
  clearMessages()
  try {
    await ElMessageBox.confirm(`确认删除出库单 ${row.orderNo} 吗？`, '删除确认', {
      type: 'warning',
